---
output:
  html_document:
    css: perrycustom.css
---
## 5 December - Day 5 - Time to put sensors on the buoy.

###This time we had the help of a new person to take photos. Amani was responsible for most of the photos in this page.

<br>
<img align="center" img src="images/day_7_sensor-19.jpg" 
height="500px" width="500px" border="1" style="PADDING-TOP: 5px" style="PADDING-BOTTOM: 5px"/>
<br>

Amani is the one that took almost all the photos and is also the computer person at TAFIRI.

<br>
<img align="center" img src="images/day_7_sensor-1.jpg" 
height="500px" width="500px" border="1" style="PADDING-TOP: 5px" style="PADDING-BOTTOM: 5px"/>
<br>

All the materials are ready to go - time to get out before the winds start to build. By afternoon the swells might be up to 0.7 m or larger again.

<br>
<img align="center" img src="images/day_7_sensor-2.jpg" 
height="500px" width="500px" border="1" style="PADDING-TOP: 5px" style="PADDING-BOTTOM: 5px"/>
<br>

Prisca and Peter discuss the plans at the fishermen beach in Kigoma.

<br>
<img align="center" img src="images/day_7_sensor-3.jpg" 
height="500px" width="500px" border="1" style="PADDING-TOP: 5px" style="PADDING-BOTTOM: 5px"/>
<br>

Prisca and Peter explain it to the rest of the crew.

<br>
<img align="center" img src="images/day_7_sensor-13.jpg" 
height="500px" width="500px" border="1" style="PADDING-TOP: 5px" style="PADDING-BOTTOM: 5px"/>
<br>

The buoy on radar. Hopefully other ships see it and don't run over it.

<br>
<img align="center" img src="images/day_7_sensor-4.jpg" 
height="500px" width="500px" border="1" style="PADDING-TOP: 5px" style="PADDING-BOTTOM: 5px"/>
<br>

The anchor floats are still on the surface and we will try to sink them down a bit tomorrow.

<br>
<img align="center" img src="images/day_7_sensor-5.jpg" 
height="500px" width="500px" border="1" style="PADDING-TOP: 5px" style="PADDING-BOTTOM: 5px"/>
<br>

The buoy is a bit bare but that will change. The lake is perfect for doing this work.

<br>
<img align="center" img src="images/day_7_sensor-6.jpg" 
height="500px" width="500px" border="1" style="PADDING-TOP: 5px" style="PADDING-BOTTOM: 5px"/>
<br>

It takes 4 - Prisca, Peter, Kodio (spelling is likley wrong - sorry), and Bill put on the lightening rod and below that the data logger is already on.

<br>
<img align="center" img src="images/day_7_sensor-7.jpg" 
height="500px" width="500px" border="1" style="PADDING-TOP: 5px" style="PADDING-BOTTOM: 5px"/>
<br>

It is not for the birds, the wind, rain, humidity and temperature sensor are all on and ready to go.

<br>
<img align="center" img src="images/day_7_sensor-8.jpg" 
height="500px" width="500px" border="1" style="PADDING-TOP: 5px" style="PADDING-BOTTOM: 5px"/>
<br>

The buoy looks a bit different now - you can just see the sensor chain being lowered.

<br>
<img align="center" img src="images/day_7_sensor-14.jpg" 
height="500px" width="500px" border="1" style="PADDING-TOP: 5px" style="PADDING-BOTTOM: 5px"/>
<br>

Lowering 150 m of sensors into the lake... only doing the shallow layers of the lake.

<br>
<img align="center" img src="images/day_7_sensor-15.jpg" 
height="500px" width="500px" border="1" style="PADDING-TOP: 5px" style="PADDING-BOTTOM: 5px"/>
<br>

Peter lowering the sensor chain.

<br>
<img align="center" img src="images/day_7_sensor-9.jpg" 
height="500px" width="500px" border="1" style="PADDING-TOP: 5px" style="PADDING-BOTTOM: 5px"/>
<br>

Time for some last minute attachments underwater of the cable to the buoy for grounding.

<br>
<img align="center" img src="images/day_7_sensor-10.jpg" 
height="500px" width="500px" border="1" style="PADDING-TOP: 5px" style="PADDING-BOTTOM: 5px"/>
<br>

Time to check the attachments

<br>
<img align="center" img src="images/day_7_sensor-11.jpg" 
height="500px" width="500px" border="1" style="PADDING-TOP: 5px" style="PADDING-BOTTOM: 5px"/>
<br>

Peter trying to hammer underwater to tighten the anchor attachments.

<br>
<img align="center" img src="images/day_7_sensor-12.jpg" 
height="500px" width="500px" border="1" style="PADDING-TOP: 5px" style="PADDING-BOTTOM: 5px"/>
<br>

The buoy in its glory.




<br>
<img align="center" img src="images/day_7_sensor-16.jpg" 
height="500px" width="500px" border="1" style="PADDING-TOP: 5px" style="PADDING-BOTTOM: 5px"/>
<br>

I think Amani wants to be captian of the boat!


<br>
<img align="center" img src="images/day_7_sensor-17.jpg" 
height="500px" width="500px" border="1" style="PADDING-TOP: 5px" style="PADDING-BOTTOM: 5px"/>
<br>

I dont think the captain, Edward, is going to be happy....

<br>
<img align="center" img src="images/day_7_sensor-18.jpg" 
height="500px" width="500px" border="1" style="PADDING-TOP: 5px" style="PADDING-BOTTOM: 5px"/>
<br>

Time to leave and let the technology do the work.

